...
{  
   ...
   if (!(t=parseFloat(window.document.input_data.drybulb.value))) 
     { 
       alert("Please enter a number for Dry Bulb temperature");
       return;
     }
   
   rh=parseFloat(window.document.input_data.rel_humidity.value);   
  
   altitude=parseFloat(window.document.input_data.altitude.value);
        
   if (t_wb=parseFloat(window.document.input_data.wetbulb.value)) 
    {
     if (t_wb>t) 
       {
   ...
   else if (t_d=parseFloat(window.document.input_data.dewpoint.value))
     {
     p = findpressure(altitude);
   ...
   if (rh<0)
     {
       window.document.input_data.rel_humidity.value=0;
       window.document.input_data.wetbulb.value = "";
       window.document.input_data.dewpoint.value = "";
       psychoCalc();
     }
   
   window.document.input_data.drybulb.value = Math.round(t*100)/100;    
   window.document.input_data.wetbulb.value = Math.round(t_wb*100)/100; 
   window.document.input_data.rel_humidity.value = Math.round(rh); 
   window.document.input_data.dewpoint.value = Math.round(t_d*100)/100; 
   window.document.input_data.altitude.value = Math.round(altitude); 
            
   window.document.results.enthalpy.value = Math.round(h*100)/100; 
   window.document.results.density.value = Math.round(rho*100)/100; 
   window.document.results.hum_ratio.value = Math.round(w*10000)/10000; 
   window.document.results.pressure.value = Math.round(p);
   window.document.results.spec_volume.value = Math.round(v*1000)/1000; 
   window.document.results.p_w.value = Math.round(p_w);
   window.document.results.p_ws.value = Math.round(p_ws); 
   return;
}
...
function setWetBulb()
{
   window.document.input_data.rel_humidity.value = "";
   window.document.input_data.dewpoint.value = "";
   psychoCalc();
   return;
}
